...
const Home = ({openModal}) => {
    return (
        <div >
            <div style={window.innerWidth < 525 ? { textAlign: 'center', fontSize: '16px' } : { paddingLeft: '40px' }} className='text-light d-flex flex-column align-items-start justify-content-center' id='cont1'>
            ...
            <div className='text-light text-center p-3' style={{background:'darkblue'}}>
                    <h1 style={ { fontSize: '36px' }} className='pt-5 w-100'>Get an account today</h1>
                    <p style={{ fontSize: '21px' }} className='pt-3 px-4 w-100'>Access free content on all of your devices, sync your list and continue watching anywhere.</p>
<button className='btn btn-light'>Register Free</button>
            </div>
        </div>
    )
}
